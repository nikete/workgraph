...
"#,
    ...
    let config = load_merged_custom(&global_dir, &local_dir);
    assert_eq!(
        config.identity.evaluator_model,
...
"#,
    ...
    // Only in global
    assert_eq!(sources.get("identity.auto_reward"), Some(&ConfigSource::Global));
    // In neither → default
...
"#,
    ...
    // identity: local evaluator_model overrides, global assigner_model inherited
    assert_eq!(config.identity.evaluator_model, Some("haiku".to_string()));
    assert_eq!(config.identity.assigner_model, Some("opus".to_string()));
    assert!(config.identity.auto_reward);
...
fn config_save_and_load_roundtrip_global() {
    ...
    config.identity.evolver_model = Some("opus".to_string());
    fs::write(
        global_dir.join("config.toml"),
    ...
    assert_eq!(loaded.identity.evolver_model, Some("opus".to_string()));
}
