...
#[test]
fn test_auto_reward_task_has_exec_and_model() {
...
"#;
    ...
    // Simulate build_auto_reward_tasks logic
    let mut mutable_graph = load_graph(&graph_path).unwrap();
    let tasks_needing_eval: Vec<_> = mutable_graph
        .tasks()
        .filter(|t| {
    ...
    for (task_id, task_title) in &tasks_needing_eval {
        ...
        let eval_task = Task {
            ...
            tags: vec!["evaluation".to_string(), "identity".to_string()],
...
fn test_eval_task_routes_to_inline_spawn() {
    ...
    eval_task.tags = vec!["evaluation".to_string(), "identity".to_string()];
