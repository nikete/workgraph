//! Integration tests for the identity federation system.
//!
...
use workgraph::identity::{
    self, Agent, IdentityStore, Reward, RewardRef, Lineage, LocalStore, Objective,
    RewardHistory, Role,
};
...
fn setup_store(tmp: &TempDir, name: &str) -> LocalStore {
    let base = tmp.path().canonicalize().unwrap_or_else(|_| tmp.path().to_path_buf());
    let path = base.join(name).join("identity");
...
fn make_evaluation(id: &str, task_id: &str, agent_id: &str, role_id: &str, objective_id: &str, value: f64) -> Reward {
    Reward {
        id: id.to_string(),
...
fn make_perf(evals: Vec<(f64, &str, &str)>) -> RewardHistory {
    let rewards: Vec<RewardRef> = evals
        .iter()
        .map(|(value, task_id, ts)| RewardRef {
            value: *value,
            task_id: task_id.to_string(),
        ...
        .collect();
    let task_count = rewards.len() as u32;
...
fn create_bare_store_dirs(dir: &Path) {
    ...
    std::fs::create_dir_all(identity.join("roles")).unwrap();
    std::fs::create_dir_all(identity.join("objectives")).unwrap();
    std::fs::create_dir_all(identity.join("agents")).unwrap();
    std::fs::create_dir_all(identity.join("rewards")).unwrap();
}

fn create_project_store_dirs(dir: &Path) {
    let identity = dir.join(".workgraph").join("identity");
    std::fs::create_dir_all(identity.join("roles")).unwrap();
    std::fs::create_dir_all(identity.join("objectives")).unwrap();
    std::fs::create_dir_all(identity.join("agents")).unwrap();
    std::fs::create_dir_all(identity.join("rewards")).unwrap();
...
/// Scanning should find project stores with .workgraph/identity/roles/.
#[test]
fn scan_finds_project_stores() {
    ...
    let store_a = LocalStore::new(proj_a.join(".workgraph").join("identity"));
    let store_b = LocalStore::new(proj_b.join(".workgraph").join("identity"));
    assert!(store_a.is_valid());
    assert!(store_b.is_valid());
}

/// Scanning should find bare stores (identity/roles/ without .workgraph parent).
#[test]
fn scan_finds_bare_stores() {
    ...
    let store = LocalStore::new(bare.join("identity"));
...
fn scan_empty_directory_no_stores() {
    let tmp = TempDir::new().unwrap();
    // An empty temp dir has no identity stores
    let store = LocalStore::new(tmp.path().join("identity"));
    assert!(!store.is_valid());
}
...
fn resolve_store_finds_project() {
    let tmp = TempDir::new().unwrap();
    let base = tmp.path().canonicalize().unwrap_or_else(|_| tmp.path().to_path_buf());
    create_project_store_dirs(&base);

    let store = federation::resolve_store(base.to_str().unwrap()).unwrap();
    assert_eq!(
        store.store_path(),
        base.join(".workgraph").join("identity")
    );
    assert!(store.is_valid());
}
...
fn resolve_store_finds_bare() {
    let tmp = TempDir::new().unwrap();
    let base = tmp.path().canonicalize().unwrap_or_else(|_| tmp.path().to_path_buf());
    create_bare_store_dirs(&base);

    let store = federation::resolve_store(base.to_str().unwrap()).unwrap();
    assert_eq!(store.store_path(), base.join("identity"));
    assert!(store.is_valid());
}
...
fn pull_existing_entity_merges_metadata() {
    ...
    // Union of 3 distinct rewards
    assert_eq!(merged.performance.rewards.len(), 3);
    assert_eq!(merged.performance.task_count, 3);
    // Target name preserved
    assert_eq!(merged.name, "analyst-local");
}

/// Pull agent → role and objective auto-pulled (referential integrity).
#[test]
...
fn pull_no_performance_strips_scores() {
    ...
    let mut objective = make_objective("m1", "quality");
    objective.performance = make_perf(vec![
    ...
    assert!(saved_role.performance.mean_reward.is_none());
    assert!(saved_role.performance.rewards.is_empty());

    let objectives = target.load_objectives().unwrap();
    let saved_mot = objectives.iter().find(|m| m.id == "m1").unwrap();
    assert_eq!(saved_mot.performance.task_count, 0);
    assert!(saved_mot.performance.mean_reward.is_none());
}
...
/// Pull with --type objectives filter.
#[test]
fn pull_type_filter_objectives_only() {
    let tmp = TempDir::new().unwrap();
    let source = setup_store(&tmp, "source");
    let target = setup_store(&tmp, "target");
    ...
    source.save_role(&make_role("r1", "analyst")).unwrap();
    source.save_objective(&make_objective("m1", "quality")).unwrap();
    source.save_objective(&make_objective("m2", "speed")).unwrap();

    let opts = TransferOptions {
        entity_filter: EntityFilter::Objectives,
        ..Default::default()
    };
    let summary = federation::transfer(&source, &target, &opts).unwrap();

    assert_eq!(summary.roles_added, 0);
    assert_eq!(summary.objectives_added, 2);
    assert!(!target.exists_role("r1"));
    assert!(target.exists_objective("m1"));
    assert!(target.exists_objective("m2"));
}
...
fn pull_entity_filter_agent_includes_deps() {
    ...
    source.save_objective(&make_objective("m1", "speed")).unwrap();
    source.save_objective(&make_objective("m2", "quality")).unwrap();
...
fn push_to_empty_target_creates_structure() {
    ...
    let target_path = tmp.path().join("target").join("identity");
...
fn push_never_deletes_from_target() {
    ...
    target.save_objective(&make_objective("m-target-only", "target-mot")).unwrap();
    ...
    assert!(target.exists_objective("m-target-only"));
...
fn remote_add_list_remove_lifecycle() {
    ...
    config.remotes.insert(
        "upstream".to_string(),
        Remote {
            path: "/some/path/identity".to_string(),
...
fn remote_show_entity_summary() {
    ...
    remote_store.save_objective(&make_objective("m1", "mot1")).unwrap();

    let counts = remote_store.entity_counts();
    assert_eq!(counts.roles, 2);
    assert_eq!(counts.objectives, 1);
...
fn merge_overlapping_entities_deduped() {
    ...
    store_a.save_objective(&make_objective("m1", "shared-mot")).unwrap();

    store_b.save_role(&make_role("r1", "shared")).unwrap();
    store_b.save_role(&make_role("r3", "b-only")).unwrap();
    store_b.save_objective(&make_objective("m1", "shared-mot")).unwrap();
    store_b.save_objective(&make_objective("m2", "b-only-mot")).unwrap();
    ...
    let objectives = target.load_objectives().unwrap();
    assert_eq!(objectives.len(), 2); // m1, m2
}
...
fn merge_into_external_target() {
    ...
    // External target
    let target_path = tmp.path().join("combined").join("identity");
    let target = LocalStore::new(&target_path);
...
fn merge_idempotent() {
    ...
    store_a.save_role(&make_role("r1", "role1")).unwrap();
    store_a.save_objective(&make_objective("m1", "mot1")).unwrap();
    store_b.save_role(&make_role("r2", "role2")).unwrap();
    store_b.save_agent(&{
        // Agent needs role and objective in same store for referential integrity
        let mut a = make_agent("a1", "agent1", "r2", "m1");
        a.role_id = "r2".to_string();
        a.objective_id = "m1".to_string();
        a
    }).unwrap();
    // Store b also needs r2 and m1 for agent deps
    store_b.save_objective(&make_objective("m1", "mot1")).unwrap();
    ...
    let mots_first = target.load_objectives().unwrap();
    ...
    let mots_second = target.load_objectives().unwrap();
...
/// Same role in two stores with different rewards → union of RewardRefs.
#[test]
fn performance_merge_union_of_evaluations() {
    ...
    let roles = target.load_roles().unwrap();
    let merged = roles.iter().find(|r| r.id == "r1").unwrap();
    assert_eq!(merged.performance.rewards.len(), 4);
...
fn performance_merge_deduplicates_same_eval() {
    ...
    let roles = target.load_roles().unwrap();
    let merged = roles.iter().find(|r| r.id == "r1").unwrap();
    assert_eq!(merged.performance.rewards.len(), 1); // deduped
    assert_eq!(merged.performance.task_count, 1);
}

/// mean_reward recalculated correctly after merge.
#[test]
fn performance_merge_mean_reward_recalculated() {
    let tmp = TempDir::new().unwrap();
    let source = setup_store(&tmp, "source");
    let target = setup_store(&tmp, "target");
    ...
    let roles = target.load_roles().unwrap();
    let merged = roles.iter().find(|r| r.id == "r1").unwrap();
    // mean_reward should be (0.9 + 0.7) / 2 = 0.8
    let avg = merged.performance.mean_reward.unwrap();
    assert!((avg - 0.8).abs() < 0.001, "Expected 0.8, got {}", avg);
}

/// Performance merge on objectives works the same way.
#[test]
fn performance_merge_objectives() {
    let tmp = TempDir::new().unwrap();
    let source = setup_store(&tmp, "source");
    let target = setup_store(&tmp, "target");

    let mut source_mot = make_objective("m1", "quality");
    source_mot.performance = make_perf(vec![
        (0.95, "task-a", "2026-01-01T00:00:00Z"),
    ]);
    source.save_objective(&source_mot).unwrap();

    let mut target_mot = make_objective("m1", "quality-local");
    target_mot.performance = make_perf(vec![
        (0.85, "task-b", "2026-01-02T00:00:00Z"),
    ]);
    target.save_objective(&target_mot).unwrap();

    federation::transfer(&source, &target, &TransferOptions::default()).unwrap();

    let objectives = target.load_objectives().unwrap();
    let merged = objectives.iter().find(|m| m.id == "m1").unwrap();
...
/// Performance merge on agents unions rewards.
#[test]
fn performance_merge_agents() {
    ...
    // Both need role + objective in store
    source.save_role(&make_role("r1", "builder")).unwrap();
    source.save_objective(&make_objective("m1", "speed")).unwrap();
    target.save_role(&make_role("r1", "builder")).unwrap();
    target.save_objective(&make_objective("m1", "speed")).unwrap();
...
/// Rewards are transferred with entities.
#[test]
fn rewards_transferred_with_entities() {
    let tmp = TempDir::new().unwrap();
    let source = setup_store(&tmp, "source");
    let target = setup_store(&tmp, "target");
    ...
    source.save_reward(&eval).unwrap();

    let summary = federation::transfer(&source, &target, &TransferOptions::default()).unwrap();
    assert_eq!(summary.rewards_added, 1);

    let evals = target.load_rewards().unwrap();
    assert_eq!(evals.len(), 1);
    assert_eq!(evals[0].id, "eval-1");
}

/// Duplicate rewards are skipped.
#[test]
fn rewards_deduped_on_transfer() {
    let tmp = TempDir::new().unwrap();
    let source = setup_store(&tmp, "source");
    let target = setup_store(&tmp, "target");
    ...
    let eval = make_evaluation("eval-1", "task-1", "a1", "r1", "m1", 0.9);
    source.save_reward(&eval).unwrap();
    target.save_reward(&eval).unwrap();

    let summary = federation::transfer(&source, &target, &TransferOptions::default()).unwrap();
    assert_eq!(summary.rewards_added, 0);
    assert_eq!(summary.rewards_skipped, 1);
}

/// no_rewards flag skips evaluation transfer.
#[test]
fn no_evaluations_flag_skips_transfer() {
    ...
    let summary = federation::transfer(&source, &target, &opts).unwrap();
    assert_eq!(summary.rewards_added, 0);
    assert_eq!(summary.rewards_skipped, 0);
...
fn transfer_from_empty_source_gives_empty_results() {
    let tmp = TempDir::new().unwrap();
    let source = LocalStore::new(tmp.path().join("empty-source").join("identity"));
    let target = setup_store(&tmp, "target");
...
fn federation_config_yaml_roundtrip() {
    ...
    config.remotes.insert(
        "upstream".to_string(),
        Remote {
            path: "/home/user/shared/identity".to_string(),
    ...
    assert_eq!(loaded.remotes["upstream"].path, "/home/user/shared/identity");
    assert_eq!(
        loaded.remotes["upstream"].description.as_deref(),
...
fn large_store_transfer() {
    ...
    // Create 50 roles, 20 objectives, 30 agents
    for i in 0..50 {
        source.save_role(&make_role(&format!("r{}", i), &format!("role-{}", i))).unwrap();
    }
    for i in 0..20 {
        source.save_objective(&make_objective(&format!("m{}", i), &format!("mot-{}", i))).unwrap();
    }
    for i in 0..30 {
...
fn ensure_store_dirs_creates_structure() {
    let tmp = TempDir::new().unwrap();
    let store_path = tmp.path().join("new-store").join("identity");
    ...
    assert!(store_path.join("objectives").is_dir());
    assert!(store_path.join("agents").is_dir());
    assert!(store_path.join("rewards").is_dir());
}
...
fn scan_max_depth_limits_discovery() {
    ...
    // Store at depth 1: root/shallow/.workgraph/identity/
    let shallow = tmp.path().join("shallow");
    create_project_store_dirs(&shallow);
    // Add a role so it's not empty
    let shallow_store = LocalStore::new(shallow.join(".workgraph").join("identity"));
    identity::init(shallow_store.store_path()).unwrap();
    shallow_store.save_role(&make_role("r-shallow", "shallow-role")).unwrap();

    // Store at depth 3: root/a/b/deep/.workgraph/identity/
    let deep = tmp.path().join("a").join("b").join("deep");
    create_project_store_dirs(&deep);
    let deep_store = LocalStore::new(deep.join(".workgraph").join("identity"));
    identity::init(deep_store.store_path()).unwrap();
...
fn push_to_read_only_directory_errors() {
    ...
    let target_path = tmp.path().join("readonly").join("identity");
...
/// Agent references role/objective not present in source → transferred without deps.
#[test]
...
/// Per design doc §7.1: missing objective in source = broken agent = error.
#[test]
fn agent_with_partial_deps() {
    ...
    // Role exists in source, objective doesn't
    source.save_role(&make_role("r1", "builder")).unwrap();
    source
...
fn force_with_no_performance_keeps_target_perf() {
    ...
    assert_eq!(merged.performance.rewards.len(), 1);
    assert_eq!(merged.performance.rewards[0].task_id, "task-tgt");
}
...
fn merge_three_stores_cascading_overlaps() {
    ...
    s1.save_objective(&make_objective("m1", "mot1")).unwrap();
    ...
    s3.save_objective(&make_objective("m1", "mot1-v2")).unwrap();
    ...
    let roles = target.load_roles().unwrap();
    assert_eq!(roles.len(), 3); // r1, r2, r3
    let mots = target.load_objectives().unwrap();
...
fn transfer_preserves_all_agent_fields() {
    ...
    assert_eq!(transferred.objective_id, "m1");
    assert_eq!(transferred.capabilities, vec!["rust", "testing"]);
    assert_eq!(transferred.executor, "amplifier");
}

/// Reward relevance filtering: only evals for transferred entities are copied.
#[test]
fn evaluation_relevance_filter_with_entity_ids() {
    ...
    // Use separate roles and objectives to avoid overlap through auto-pulled deps
    source.save_role(&make_role("r1", "builder")).unwrap();
    source.save_role(&make_role("r2", "tester")).unwrap();
    source.save_objective(&make_objective("m1", "speed")).unwrap();
    source.save_objective(&make_objective("m2", "quality")).unwrap();
    ...
    // Eval for a1 and a2
    source.save_reward(&make_evaluation("eval-1", "task-1", "a1", "r1", "m1", 0.9)).unwrap();
    source.save_reward(&make_evaluation("eval-2", "task-2", "a2", "r2", "m2", 0.8)).unwrap();
    ...
    assert_eq!(summary.agents_added, 1);
    assert_eq!(summary.rewards_added, 1); // only eval-1 (for a1/r1/m1)
...
fn scan_cli_finds_multiple_stores_json() {
    ...
    create_project_store_dirs(&proj_a);
    let store_a = LocalStore::new(proj_a.join(".workgraph").join("identity"));
    identity::init(store_a.store_path()).unwrap();
    store_a.save_role(&make_role("r1", "role-alpha")).unwrap();

    let proj_b = tmp.path().join("beta");
    create_project_store_dirs(&proj_b);
    let store_b = LocalStore::new(proj_b.join(".workgraph").join("identity"));
    identity::init(store_b.store_path()).unwrap();
    store_b.save_role(&make_role("r2", "role-beta")).unwrap();
    store_b.save_objective(&make_objective("m1", "mot-beta")).unwrap();
    ...
    assert_eq!(counts_b.objectives, 1);
}
...
fn scan_mixed_bare_and_project_stores() {
    ...
    let store_proj = LocalStore::new(proj.join(".workgraph").join("identity"));
    identity::init(store_proj.store_path()).unwrap();
    store_proj.save_role(&make_role("r1", "proj-role")).unwrap();

    // Bare store
    let bare = tmp.path().join("shared");
    create_bare_store_dirs(&bare);
    let store_bare = LocalStore::new(bare.join("identity"));
    identity::init(store_bare.store_path()).unwrap();
...
fn transfer_preserves_role_skills() {
    ...
    role.skills = vec![
        workgraph::identity::SkillRef::Name("rust".to_string()),
        workgraph::identity::SkillRef::Name("testing".to_string()),
        workgraph::identity::SkillRef::Inline("custom skill content".to_string()),
    ];
    source.save_role(&role).unwrap();
...
/// Transfer preserves acceptable and unacceptable tradeoffs on objectives.
#[test]
fn transfer_preserves_objective_tradeoffs() {
    let tmp = TempDir::new().unwrap();
    let source = setup_store(&tmp, "source");
    let target = setup_store(&tmp, "target");

    let mut objective = make_objective("m1", "quality");
    objective.acceptable_tradeoffs = vec![
        "Slower delivery for higher quality".to_string(),
        "More verbose output".to_string(),
    ];
    objective.unacceptable_tradeoffs = vec![
    ...
    let objectives = target.load_objectives().unwrap();
    let transferred = objectives.iter().find(|m| m.id == "m1").unwrap();
...
/// Context_id in RewardRef is preserved through performance merge.
#[test]
fn performance_merge_preserves_context_id() {
    ...
    source_role.performance = RewardHistory {
        ...
        rewards: vec![RewardRef {
            ...
            context_id: "objective-xyz".to_string(),
        }],
    };
    source.save_role(&source_role).unwrap();
    ...
    target_role.performance = RewardHistory {
        ...
        rewards: vec![RewardRef {
            ...
            context_id: "objective-abc".to_string(),
        }],
    };
    target.save_role(&target_role).unwrap();
    ...
    let has_xyz = merged.performance.rewards.iter().any(|e| e.context_id == "objective-xyz");
    let has_abc = merged.performance.rewards.iter().any(|e| e.context_id == "objective-abc");
    assert!(has_xyz, "context_id 'objective-xyz' should be preserved");
    assert!(has_abc, "context_id 'objective-abc' should be preserved");
}
...
/// Force flag on objective overwrites target metadata.
#[test]
fn force_flag_overwrites_objective() {
    let tmp = TempDir::new().unwrap();
    let source = setup_store(&tmp, "source");
    let target = setup_store(&tmp, "target");

    let mut source_mot = make_objective("m1", "source-quality");
    source_mot.description = "Source description".to_string();
    source_mot.acceptable_tradeoffs = vec!["source tradeoff".to_string()];
    source.save_objective(&source_mot).unwrap();

    let mut target_mot = make_objective("m1", "target-quality");
    ...
    let summary = federation::transfer(&source, &target, &opts).unwrap();
    assert_eq!(summary.objectives_updated, 1);
...
/// No_rewards + force: overwrites metadata, keeps rewards out.
#[test]
...
fn merge_three_stores_performance_accumulation() {
    ...
    // All three have the same role with different rewards
...
fn merge_idempotent_with_performance() {
    ...
    let roles = target.load_roles().unwrap();
    let merged = roles.iter().find(|r| r.id == "r1").unwrap();
    assert_eq!(merged.performance.rewards.len(), 2); // Not duplicated
}
...
/// Agent whose role_id and objective_id exist in target but not source:
...
/// Two agents sharing the same role but different objectives: both deps pulled.
#[test]
...
/// Multiple rewards from same agent, different tasks → all transferred.
#[test]
fn multiple_evaluations_same_agent_all_transferred() {
    ...
    let summary = federation::transfer(&source, &target, &TransferOptions::default()).unwrap();
    assert_eq!(summary.rewards_added, 5);

    let evals = target.load_rewards().unwrap();
    assert_eq!(evals.len(), 5);
}

/// Reward dimensions (HashMap) preserved through transfer.
#[test]
...
/// Rewards from two sources, partially overlapping, merged into target.
#[test]
fn rewards_merge_from_two_sources() {
    let tmp = TempDir::new().unwrap();
    let s1 = setup_store(&tmp, "s1");
    let s2 = setup_store(&tmp, "s2");
    let target = setup_store(&tmp, "target");
    ...
    s1.save_reward(&make_evaluation("eval-1", "task-1", "a1", "r1", "m1", 0.9)).unwrap();
    s1.save_reward(&make_evaluation("eval-2", "task-2", "a1", "r1", "m1", 0.8)).unwrap();

    // s2 has eval-2 (overlap) and eval-3
    s2.save_reward(&make_evaluation("eval-2", "task-2", "a1", "r1", "m1", 0.8)).unwrap();
    s2.save_reward(&make_evaluation("eval-3", "task-3", "a1", "r1", "m1", 0.7)).unwrap();
    ...
    assert_eq!(sum1.rewards_added, 2);
    assert_eq!(sum2.rewards_added, 1); // eval-2 already exists
    assert_eq!(sum2.rewards_skipped, 1); // eval-2 skipped
...
#[test]
fn resolve_store_direct_identity_path() {
    let tmp = TempDir::new().unwrap();
    let base = tmp.path().canonicalize().unwrap_or_else(|_| tmp.path().to_path_buf());
    let identity_dir = base.join("identity");
    std::fs::create_dir_all(identity_dir.join("roles")).unwrap();
    std::fs::create_dir_all(identity_dir.join("objectives")).unwrap();
    std::fs::create_dir_all(identity_dir.join("agents")).unwrap();
    std::fs::create_dir_all(identity_dir.join("rewards")).unwrap();

    // Point directly at the identity dir
    let store = federation::resolve_store(identity_dir.to_str().unwrap()).unwrap();
    assert_eq!(store.store_path(), identity_dir);
    assert!(store.is_valid());
}
...
fn transfer_empty_to_empty_all_zeros() {
    ...
    assert_eq!(summary.objectives_added, 0);
    assert_eq!(summary.objectives_updated, 0);
    assert_eq!(summary.objectives_skipped, 0);
    assert_eq!(summary.agents_added, 0);
...
fn accumulate(total: &mut TransferSummary, part: &TransferSummary) {
    ...
    total.objectives_added += part.objectives_added;
    total.objectives_updated += part.objectives_updated;
    total.objectives_skipped += part.objectives_skipped;
    total.agents_added += part.agents_added;
    total.agents_updated += part.agents_updated;
    total.agents_skipped += part.agents_skipped;
    total.rewards_added += part.rewards_added;
    total.rewards_skipped += part.rewards_skipped;
}
