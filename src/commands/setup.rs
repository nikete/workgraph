...
pub fn build_config(choices: &SetupChoices, base: Option<&Config>) -> Config {
    ...
    config.identity.auto_assign = choices.agency_enabled;
    config.identity.auto_reward = choices.agency_enabled;

    if let Some(ref eval_model) = choices.evaluator_model {
        config.identity.evaluator_model = Some(eval_model.clone());
    }
    if let Some(ref assign_model) = choices.assigner_model {
        config.identity.assigner_model = Some(assign_model.clone());
    }
...
pub fn format_summary(choices: &SetupChoices) -> String {
    ...
    lines.push(format!("  auto_reward = {}", choices.agency_enabled));
...
pub fn run() -> Result<()> {
    ...
    let agency_enabled = Confirm::new()
        ...
        .default(existing.identity.auto_assign || existing.identity.auto_reward)
        .interact()?;

    let (evaluator_model, assigner_model) = if agency_enabled {
        // Evaluator model
        let eval_options = &["sonnet (recommended)", "haiku", "same as default"];
        let current_eval_idx = match existing.identity.evaluator_model.as_deref() {
            Some("haiku") => 1,
            Some(m) if m == model => 2,
            _ => 0,
        };
        ...
        let current_assign_idx = match existing.identity.assigner_model.as_deref() {
            Some("sonnet") => 1,
            Some(m) if m == model => 2,
            _ => 0,
        };
...
mod tests {
    ...
    fn test_build_config_defaults() {
        ...
        assert_eq!(config.identity.evaluator_model, Some("sonnet".to_string()));
    ...
    fn test_build_config_preserves_base() {
        ...
        base.identity.retention_heuristics = Some("keep good ones".to_string());
    ...
    fn test_format_summary_basic() {
        ...
        assert!(summary.contains("auto_reward = true"));
    ...
    fn test_format_summary_agency_disabled() {
        ...
        assert!(summary.contains("auto_reward = false"));
    ...
    fn test_build_config_roundtrip_through_toml() {
        ...
        assert!(reloaded.identity.auto_assign);
        assert!(reloaded.identity.auto_reward);
        assert_eq!(reloaded.identity.evaluator_model, Some("sonnet".to_string()));
        assert_eq!(reloaded.identity.assigner_model, Some("haiku".to_string()));
    }
