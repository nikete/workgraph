...
pub fn run(dir: &Path, task_id: &str, json: bool) -> Result<()> {
    ...
    // Load agents from .workgraph/identity/agents/
    let agents_dir = dir.join("identity").join("agents");
    let agents = identity::load_all_agents(&agents_dir).context("Failed to load agents")?;
    ...
    if json {
        ...
        if matches.is_empty() {
            ...
            for m in &matches {
                ...
                let short_id = identity::short_hash(&m.agent_id);
...
mod tests {
    ...
    fn test_match_no_agents_registered() {
        ...
        // Initialize identity but add no agents
