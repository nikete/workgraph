...
pub fn run(workgraph_dir: &Path, opts: &MergeOptions) -> Result<()> {
    ...
    let target = if let Some(ref into_path) = opts.into {
        ...
        let store = LocalStore::new(&identity_dir);
        federation::ensure_store_dirs(&store)?;
        store
    };
    ...
    if opts.json {
        let output = serde_json::json!({
            ...
            "objectives": {
                "added": total.objectives_added,
                "updated": total.objectives_updated,
                "skipped": total.objectives_skipped,
            },
            "agents": {
            ...
            "rewards": {
                "added": total.rewards_added,
                "skipped": total.rewards_skipped,
            },
        });
        ...
        let total_objectives =
            total.objectives_added + total.objectives_updated + total.objectives_skipped;
        ...
        println!(
            "  Objective transfers: {} ({} new, {} existing)",
            total_objectives,
            total.objectives_added,
            total.objectives_updated + total.objectives_skipped
        );
        println!(
        ...
        if total.rewards_added > 0 || total.rewards_skipped > 0 {
            println!(
                "  Rewards: +{} new, {} skipped",
                total.rewards_added, total.rewards_skipped
            );
        }
    }
...
mod tests {
    ...
    fn merge_two_stores_with_overlapping_and_unique_entities() {
        ...
        store_a
            .save_objective(&make_objective("m1", "shared-mot"))
            .unwrap();
        ...
        store_b
            ...
            .unwrap();
        store_b
            .save_objective(&make_objective("m2", "b-only-mot"))
            .unwrap();
        ...
        // Should have all unique objectives: m1, m2
        assert!(result.exists_objective("m1"));
        assert!(result.exists_objective("m2"));
    }
    ...
    fn merge_into_target_path() {
        ...
        // The target should be at into_dir/identity/ (bare store convention)
    ...
    fn merge_idempotent() {
        ...
        store_a.save_role(&make_role("r1", "role1")).unwrap();
        store_a
            .save_objective(&make_objective("m1", "mot1"))
            .unwrap();
        store_b.save_role(&make_role("r2", "role2")).unwrap();
        ...
        let mots_after_first = result.load_objectives().unwrap();

        // Second merge (idempotent)
        run(&wg_dir, &opts).unwrap();

        let roles_after_second = result.load_roles().unwrap();
        let mots_after_second = result.load_objectives().unwrap();
    ...
    fn merge_performance_records_unioned() {
        ...
        role_a.performance = RewardHistory {
            task_count: 1,
        ...
        role_b.performance = RewardHistory {
            task_count: 1,
    ...
    fn merge_with_agents_pulls_dependencies() {
        ...
        // Store A has role + objective + agent
        ...
        // Store B has a different role + objective + agent
        store_b.save_role(&make_role("r2", "tester")).unwrap();
        store_b
            .save_objective(&make_objective("m2", "quality"))
            .unwrap();
        store_b
