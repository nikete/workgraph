...
use workgraph::identity::IdentityStore;
...
pub fn run_show(workgraph_dir: &Path, name: &str, json: bool) -> Result<()> {
    ...
    if json {
        ...
        if let Ok(store) = &store_result {
            if store.is_valid() {
                ...
                let objectives = store.load_objectives().unwrap_or_default();
                let agents = store.load_agents().unwrap_or_default();
                let evaluations = store.load_rewards().unwrap_or_default();
    ...
    match store_result {
        Ok(store) if store.is_valid() => {
            ...
            let evaluations = store.load_rewards().unwrap_or_default();
            println!(
                "  Entities:    {} roles, {} objectives, {} agents, {} evaluations",
                roles.len(),
                objectives.len(),
                agents.len(),
                evaluations.len()
...
mod tests {
    ...
    use workgraph::identity::LocalStore;
    ...
    fn setup_remote_store(tmp: &TempDir, name: &str) -> LocalStore {
        let path = tmp.path().canonicalize().unwrap().join(name).join("identity");
    ...
    fn show_remote_with_valid_store() {
        ...
        // Add some entities
        use workgraph::identity::{Lineage, RewardHistory, Role};
        store
            .save_role(&Role {
                id: "r1".to_string(),
