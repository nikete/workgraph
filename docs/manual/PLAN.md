# Workgraph + Identity Manual — Document Plan

This plan governs the structure, terminology, tone, and content of a conceptual manual for humans who want to understand and use workgraph and its identity system for their own projects. This is not an API reference or CLI cheat-sheet. It is a book — tight, precise prose that builds understanding from first principles.
...
| **coordinator** | The scheduling brain inside the service daemon. Runs a tick loop that: cleans up dead agents, counts available slots, optionally creates auto-assign and auto-reward meta-tasks, finds ready tasks, and spawns agents for them. |
...
| **assignment** | Binding an identity agent identity to a task (via `wg assign`). When the task is dispatched, the agent's role and objective are rendered into the prompt. Distinct from *claiming* — assignment sets identity, claiming sets execution state. |
| **auto-assign** | A coordinator feature that automatically creates `assign-{task-id}` meta-tasks for unassigned ready work. An assigner agent (itself an identity entity) rewards available agents and picks the best fit. |
| **auto-reward** | A coordinator feature that automatically creates `reward-{task-id}` meta-tasks for completed work. The reward task runs `wg reward`, which spawns an evaluator agent to score the work. |
| **reward** | A scored assessment of an agent's work on a task. Four dimensions: *correctness* (40%), *completeness* (30%), *efficiency* (15%), *style adherence* (15%). Produces an overall weighted score (0.0–1.0). Scores propagate to three levels: the agent, its role, and its objective. |
| **performance record** | A running tally on each agent, role, and objective: task count, average score, and a list of reward references. Role rewards carry `context_id = objective_id` and vice versa, enabling synergy analysis. |
| **evolution** | The process of improving identity entities based on reward data. `wg evolve` spawns an evolver agent that proposes structured operations (create, modify, retire) on roles and objectives. |
| **strategy** | An evolution approach: *mutation* (modify one entity), *crossover* (combine two), *gap-analysis* (create for unmet needs), *retirement* (remove underperformers), *objective-tuning* (adjust trade-offs), or *all* (use all strategies). |
| **lineage** | Evolutionary history tracked on every role, objective, and agent. Records parent IDs (empty for manual creation, one for mutation, two for crossover), generation number, creator identity, and timestamp. |
...
| **synergy matrix** | A performance cross-reference showing how each (role, objective) pair performs together. Computed from the `context_id` fields in reward references. Surfaced by `wg identity stats`. |
| **meta-task** | A task created automatically by the coordinator to manage the identity loop. Assignment tasks (`assign-{id}`), reward tasks (`reward-{id}`), and evolution review tasks are meta-tasks. Tagged to prevent recursive meta-task creation. |
...
**Purpose:** Establish what workgraph is, what the identity system is, how they relate, and why this combination exists. The reader should finish this section with a clear mental model of the whole before diving into parts.
...
**Cross-references:** Forward-references to Section 2 (task graph), Section 3 (identity model), Section 4 (coordination).
...
**Cross-references:** Back-reference to Section 1 (overview). Forward-reference to Section 4 (how the coordinator uses readiness). Forward-reference to Section 5 (loop edges as reward points).
...
**File:** `docs/manual/03-identity.typ`

**Purpose:** Explain the identity system — roles, objectives, agents, content-hash IDs, the skill system, capabilities, trust, and how human and AI agents coexist. The reader should finish this section able to design an identity for their own project.

**Key points to cover:**

1. **Why composable identities.** A generic AI assistant is a blank slate — it has no persistent personality, no declared priorities, no way to improve. The identity system solves this by giving agents a *what* (role) and a *why* (objective). Same role + different objective = different agent. This combinatorial identity space is the key insight.
...
3. **Objectives.** Why an agent acts the way it does. Fields: name, description, acceptable trade-offs (what it may sacrifice), unacceptable trade-offs (hard constraints). Description and trade-offs are identity-defining. Objectives shape *how* the agent approaches work — a "Careful" objective produces different behavior than a "Fast" objective on the same role.

4. **Agents: the pairing.** An AI agent is `(role_id, objective_id)` — its ID is the content-hash of that pair. A human agent may omit role and objective. The agent struct also carries operational fields: capabilities (for routing), rate (for cost), capacity (for parallelism), trust level (for priority), contact info, and executor (for dispatch).
...
**Purpose:** Explain how work gets dispatched and monitored — the service daemon, the coordinator loop, agent spawning, parallelism, the full dispatch cycle, auto-assign, auto-reward, and dead agent handling. The reader should finish this section able to run a multi-agent project.
...
2. **The coordinator tick.** The six-phase heartbeat: (1) reap zombie processes, (2) clean up dead agents and count alive slots, (3) create auto-assign meta-tasks if enabled, (4) create auto-reward meta-tasks if enabled, (5) save graph if modified and find ready tasks, (6) spawn agents for ready tasks up to available slots. Two triggers: IPC-driven (immediate, reactive) and safety-net poll (background timer, catches manual edits).
...
6. **Auto-assign.** When enabled, the coordinator creates blocking `assign-{task-id}` meta-tasks for unassigned ready work. An assigner agent (configurable model and identity) rewards available agents and picks the best fit. Meta-tasks are tagged to prevent recursive auto-assignment.

7. **Auto-reward.** When enabled, the coordinator creates `reward-{task-id}` meta-tasks blocked by each work task. When the work task completes (or fails), the reward task becomes ready. Reward tasks use the shell executor to run `wg reward`. Human-agent tasks are skipped. Meta-tasks are tagged to prevent recursive reward.
...
**Cross-references:** Back-reference to Section 2 (readiness). Back-reference to Section 3 (agent identity injection). Forward-reference to Section 5 (reward feeding evolution).
...
**Purpose:** Explain how the identity improves over time — reward, evolution strategies, lineage, performance aggregation, the synergy matrix, and the autopoietic nature of the system. The reader should finish this section understanding how to run the improvement cycle and what to expect from it.

**Key points to cover:**

1. **The identity as a living system.** The full loop: assign → execute → reward → evolve → assign (improved). Each step feeds the next. The system is designed to learn from its own performance data and produce better agent identities over time. This is not magic — it is a structured feedback loop with human oversight at the evolution step.

2. **Reward in depth.** What the evaluator sees: task definition, agent identity, artifacts, log entries, timing. What it scores: correctness (40%), completeness (30%), efficiency (15%), style adherence (15%). How scores are computed: weighted average to a single 0.0–1.0 score. How scores propagate: to the agent, the role (with objective as context), and the objective (with role as context). This three-level propagation creates the data needed for synergy analysis.

3. **The performance record.** Each entity maintains: task count, average score, and a list of reward references. The `context_id` on each reward reference enables cross-cutting analysis: "how does this role perform with different objectives?" and vice versa.

4. **The synergy matrix.** A performance cross-reference computed from context IDs. For every (role, objective) pair with rewards, the matrix shows average score and count. High-synergy pairs (score >= 0.8) and low-synergy pairs (score <= 0.4) are flagged. Under-explored combinations (fewer than N rewards) are surfaced for experimentation.

5. **Trend indicators.** `wg identity stats` computes trends by comparing first and second halves of recent scores: improving, declining, or flat. These trends guide evolution decisions.

6. **Evolution strategies.** Six approaches the evolver can use: *Mutation* (modify one existing role to improve weak dimensions), *Crossover* (combine traits from two high-performing roles), *Gap analysis* (create entirely new roles/objectives for unmet needs), *Retirement* (remove consistently poor performers), *Objective tuning* (adjust trade-offs on existing objectives), and *All* (use all strategies as appropriate). Each strategy has optional guidance documents in `.workgraph/identity/evolver-skills/`.

7. **How evolution works mechanically.** The evolver agent receives: performance summaries, strategy instructions, budget constraints, retention heuristics, and its own identity (if configured). It outputs structured JSON operations: create_role, modify_role, create_objective, modify_objective, retire_role, retire_objective. Operations are applied to the identity storage. Modified entities get new content-hash IDs with lineage metadata linking to their parents.

8. **Safety guardrails.** The last remaining role or objective cannot be retired. Retired entities are preserved as `.yaml.retired` files, not deleted. `--dry-run` shows the evolver's plan without applying. `--budget` limits operations per run. Self-mutations (evolver changing its own role/objective) are deferred to human review via a workgraph task with verification requirements.
...
**Cross-references:** Back-reference to Section 3 (roles, objectives, content-hash IDs). Back-reference to Section 4 (auto-reward, auto-assign creating the data pipeline). Back-reference to Section 2 (loop edges as natural reward points for iterative tasks).

**Tone notes:** This section should feel like watching a system learn. The prose should build from the concrete (one reward) to the systemic (the full improvement cycle). End with the philosophical point: this is a system that can describe and improve itself, but always with a human hand on the wheel.
...
| 01-overview | 03-identity | "The identity system is detailed in §3" |
...
| 05-evolution | 03-identity | "Roles, objectives, content-hash IDs — §3" |
| 05-evolution | 04-coordination | "Auto-reward and auto-assign — §4" |
| 05-evolution | 02-task-graph | "Loop edges as reward points — §2" |
...
5. **"Agent" means the identity entity, not the running process.** The running process is an "agent process" or simply "process." The identity is the "agent." When in doubt, say "agent identity" for the entity and "spawned agent" or "running agent" for the process.

6. **"Assign" vs. "claim."** Assigning binds an identity to a task. Claiming marks a task as in-progress. These are different operations. The coordinator claims; the identity system (or user) assigns.
...
9. **Use "role" and "objective" as nouns, never as adjectives.** Say "the agent's role" not "the role agent." Say "the objective's trade-offs" not "the objective trade-offs."
...
12. **"Meta-task" for auto-created coordinator tasks.** Assignment tasks, reward tasks, and evolution review tasks are meta-tasks. Regular work is just "tasks."
...
| `docs/manual/03-identity.typ` | The Identity Model | Identity system. ~2500-3500 words. |
...
- **Example-driven.** Walk through concrete examples. The review-revise loop, the CI retry pipeline, the four-role identity — these should recur across sections to build familiarity.
